...
# ══════════════════════════════════════════════════════════════════════════════
# EXAMPLE — generates all 9 charts from hardcoded values
# Replace with your real numbers as you get them
# ══════════════════════════════════════════════════════════════════════════════
if __name__ == "__main__":

    apply_global_style()
    FIGURES_DIR = "/Users/aadya/Documents/script_interp/Figures"

    results = {
        # (model, task): {script: accuracy}
        ("Qwen3-4B",     "NER"): {"Devanagari": 70.9, "Roman": 75.4, "Mixed": 52.6},
        ("Llama-3.2-3B", "NER"): {"Devanagari": 71.4, "Roman": 76.8, "Mixed": 66.2},
        ("Gemma-2-2B",   "NER"): {"Devanagari": 70.0, "Roman": 73.8, "Mixed": 41.0},
        # fill in POS and NLI when ready:
        ("Qwen3-4B",     "POS"): {"Devanagari": 0.0,  "Roman": 0.0,  "Mixed": 0.0},
        ("Llama-3.2-3B", "POS"): {"Devanagari": 0.0,  "Roman": 0.0,  "Mixed": 0.0},
        ("Gemma-2-2B",   "POS"): {"Devanagari": 0.0,  "Roman": 0.0,  "Mixed": 0.0},
        ("Qwen3-4B",     "NLI"): {"Devanagari": 0.0,  "Roman": 0.0,  "Mixed": 0.0},
        ("Llama-3.2-3B", "NLI"): {"Devanagari": 0.0,  "Roman": 0.0,  "Mixed": 0.0},
        ("Gemma-2-2B",   "NLI"): {"Devanagari": 0.0,  "Roman": 0.0,  "Mixed": 0.0},
    }

    for (model, task), acc in results.items():
        ylabel = "Token-Level Accuracy (%)" if task == "POS" else "Accuracy (%)"
        plot_accuracy_bars(
            accuracy   = acc,
            model_name = model,
            task_name  = task,
...
# ── assumes plot_accuracy.py was already run in a previous cell ───────────────
# If not, paste and run plot_accuracy.py first, then run this cell.
# apply_global_style() and plot_accuracy_bars() will be available in memory.
apply_global_style()

# ── CONFIG ────────────────────────────────────────────────────────────────────
INPUT_FILE   = "/Users/aadya/Documents/script_interp/indic_mixed_dataset.jsonl"
OUTPUT_CSV   = "/Users/aadya/Documents/script_interp/nli_results.csv"
FIGURES_DIR  = "/Users/aadya/Documents/script_interp/Figures"
MODEL_NAME   = "meta-llama/Llama-3.2-3B-Instruct" # Updated for Llama 3.2
NUM_SAMPLES  = 1000
# ─────────────────────────────────────────────────────────────────────────────
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    attn_implementation="eager",  # <--- THIS IS THE MAGIC FIX
...
def predict(premise, hypothesis):
    ...
    text = tokenizer.apply_chat_template(
        ...
        # enable_thinking=False  <-- Removed: Llama does not support this arg
    )
...
plot_accuracy_bars(
    ...
    model_name = "Llama-3.2-3B", # Updated for plot title/save logic
    task_name  = "NLI",
    save_dir   = FIGURES_DIR,
)